...
> ⚠️ **Requires a [Nerd Font](https://www.nerdfonts.com/)** installed and set as your terminal font. Without it, most icons will appear as missing glyphs.
## Features
### Multi-line layout
...
## Customisation tips
- **Enable the clock**: find `[time]`, add `disabled = false`, then add `$time` to the `format` string where you want it to appear (e.g. on the first line after `$cmd_duration`).
